...
import { Sparkles, X, Dna, Search, Brain, Loader2, Check, RefreshCw, Edit, ExternalLink } from "lucide-react"
...
export function AIModal() {
  ...
  const handleGenerate = async () => {
    ...
    const stages = [
      { progress: 10, message: "Initializing AudioGenetics engine..." },
      { progress: 20, message: "Analyzing musical genome..." },
      { progress: 35, message: "Researching lyrical patterns..." },
      { progress: 50, message: "Processing emotional DNA..." },
      { progress: 65, message: "Composing original verses..." },
      { progress: 80, message: "Crafting hooks and bridges..." },
      { progress: 95, message: "Finalizing Billboard-caliber composition..." },
    ]
    ...
    try {
      const response = await fetch("/api/audiogenetics/generate", {
        ...
        body: JSON.stringify({
          ...
          searchResults: analysisData?.searchResults,
        }),
      })
  ...
  const handleAccept = () => {
    ...
    const title = generatedResult.metadata.songTitle || localConfig.songTitle || "Generated Song"

    if (currentFileId) {
      updateFile(currentFileId, {
        content: generatedResult.lyrics,
        title,
        musical_influence: localConfig.musicalInfluence,
        subject_matter: localConfig.subjectMatter,
      })
    } else {
      createFile(title, generatedResult.lyrics)
    }

    setEditorContent(generatedResult.lyrics)
    setShowResults(false)
    ...
    <AnimatePresence>
      ...
      >
        ...
        <motion.div
          ...
          onClick={() => !isGenerating && !showResults && setIsAIModalOpen(false)}
        />
        ...
        <motion.div
          ...
          className="glass-panel relative z-10 max-h-[90vh] w-full max-w-4xl overflow-y-auto scrollbar-thin"
        >
          {showResults && generatedResult ? (
            <div className="p-6">
              <div className="mb-6 flex items-center justify-between border-b border-white/5 pb-4">
                <div>
                  <h2 className="text-xl font-semibold text-white">Generated Song Preview</h2>
                  <p className="text-sm text-white/50">Review your Billboard-caliber composition</p>
                </div>
              ...
              {/* BTS Insights */}
              {generatedResult.metadata.analysisData && (
                <motion.div
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ opacity: 1, y: 0 }}
                  className="mb-6 rounded-xl border border-neon-cyan/20 bg-neon-cyan/5 p-4"
                >
                  <div className="mb-3 flex items-center gap-2">
                    <Brain className="h-4 w-4 text-neon-cyan" />
                    <span className="text-sm font-medium text-neon-cyan">
                      AudioGenetics Analysis (Behind the Scenes)
                    </span>
                  </div>
                  <div className="grid gap-2 text-xs">
                    <div className="flex justify-between">
                      <span className="text-white/50">Emotional Tone:</span>
                      <span className="text-white">{generatedResult.metadata.analysisData.emotionalTone}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-white/50">Lyrical Themes:</span>
                      <span className="text-white">{generatedResult.metadata.analysisData.lyricalThemes}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-white/50">Writing Style:</span>
                      <span className="text-white">{generatedResult.metadata.analysisData.writingStyle}</span>
                    </div>
                  </div>
                  {generatedResult.metadata.searchResults && generatedResult.metadata.searchResults.length > 0 && (
                    <div className="mt-3 border-t border-white/10 pt-3">
                      <span className="text-xs text-white/50">Research URLs:</span>
                      <div className="mt-2 space-y-1">
                        {generatedResult.metadata.searchResults.map((url, i) => (
                          <a
                            key={i}
                            href={url}
                            target="_blank"
              ...
              <div className="mb-4 grid gap-3 md:grid-cols-2">
                {generatedResult.metadata.songTitle && (
                  <div className="rounded-lg border border-white/10 bg-white/5 p-3">
                    <span className="text-xs text-white/50">Song Title</span>
                    <p className="mt-1 font-semibold text-white">{generatedResult.metadata.songTitle}</p>
                  </div>
                )}
                {generatedResult.metadata.audioTag && (
                  <div className="rounded-lg border border-white/10 bg-white/5 p-3">
                    <span className="text-xs text-white/50">Audio Tag</span>
                    <p className="mt-1 text-white">{generatedResult.metadata.audioTag}</p>
                  </div>
                )}
              </div>

              {generatedResult.metadata.imagePrompt && (
                <div className="mb-4 rounded-lg border border-white/10 bg-white/5 p-3">
                  <span className="text-xs text-white/50">Cover Art Prompt</span>
                  <p className="mt-1 text-sm text-white">{generatedResult.metadata.imagePrompt}</p>
                </div>
              )}

              {/* Lyrics Preview */}
              <div className="mb-6 max-h-96 overflow-y-auto rounded-xl border border-white/10 bg-black/30 p-4">
                <pre className="whitespace-pre-wrap font-mono text-sm text-white">{generatedResult.lyrics}</pre>
              </div>
              ...
              <div className="grid gap-3 sm:grid-cols-4">
                ...
                <Button
                  ...
                  variant="outline"
                  className="gap-2 border-neon-purple/30 bg-neon-purple/10 text-neon-purple hover:bg-neon-purple/20"
                >
            ...
            <>
              ...
              <div className="flex items-center justify-between border-b border-white/5 px-6 py-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-white/50">Billboard Top 10 Lyric Generator</p>
                  </div>
                </div>
                <Button
                  variant="ghost"
              ...
              <div className="space-y-6 p-6">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-white/40">Click "Analyze" to extract musical genome and research URLs</p>
                </div>
...
function getProgressMessage(progress: number): string {
  if (progress < 10) return "Initializing AudioGenetics engine..."
  if (progress < 20) return "Analyzing musical genome..."
  if (progress < 35) return "Researching lyrical patterns..."
  if (progress < 50) return "Processing emotional DNA..."
  if (progress < 65) return "Composing original verses..."
  if (progress < 80) return "Crafting hooks and bridges..."
  if (progress < 100) return "Finalizing Billboard-caliber composition..."
  return "Complete!"
}
